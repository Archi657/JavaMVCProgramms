...
#include <iomanip>
#include <math.h>
#include <tuple> //Para usar esta libreria es necesario compilar siguiendo el estandar de C++11

using namespace std;

const int grado=3;
//float p []={1,-3.5,2.75,2.125,-3.875,1.25};
float p []={1,-4,5.25,-2.5};
//float p []={40,-20};
...
std::tuple<float, float, int> rs(float r, float s){
    ...
    do{
        ...
        s+=ds;
        it++;
        //-cout << "r: " << r << " s: "<< s << endl;          }while((fabs(dr/r)>pow(10,-6))||(fabs(ds/s)>pow(10,-6)));
...
int main()
{
    float r, s, x1, x2;
    int it;
    cout << "Ingrese dos valores iniciales" << endl;     cin >> r;
    cin >> s;
    tie(r,s,it)= rs(r,s);
    cout << "r: " << r << " s: "<< s << endl;
